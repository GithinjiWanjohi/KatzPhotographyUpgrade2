...
if(isset($_POST['accept'])){
    $accSql=mysqli_query($db, "UPDATE `book_class` SET `status`='Accepted' WHERE `id`='$classid'");
    //header("Refresh:0; url=bookingRequest.php");
}
if(isset($_POST['decline'])){
    $decSql=mysqli_query($db, "UPDATE `book_class` SET `status`='Declined' WHERE `id`='$classid'");
    //header("Refresh:0; url=bookingRequest.php");    
}
?>
                    ...
                    <div class="form-box clearfix">
                        <div class="col-sm-9">
                          ...
                          <table class="table">
                            <thead>
                              <tr>
                                <th>Name</th><th>Email</th><th>Phonenumber</th><th>Details</th><th>Time</th><th>Status</th><th></th><th></th>
                              </tr>
                            </thead>
                            <tbody>

                          <?php
                            $shootRequestSql=mysqli_query($db, "SELECT * FROM `book_class`");
                              while ($rowE=mysqli_fetch_array($shootRequestSql)) {
                                ...
                                $date=$datetime[0];
                                $time=$datetime[1];
                                $rtime=date('h:i a',strtotime($time));
                                $rdate=date('d M Y',strtotime($date));

                          ?>
                              <tr>
                                <td><?php echo $name; ?></td><td><?php echo $email; ?></td><td><?php echo $phonenumber; ?></td><td><?php echo $details; ?></td><td><?php echo $rtime." ".$rdate; ?></td><td><?php echo $Status; ?></td>
                                <td>
                                    <form method="POST" action="bookingclass.php?cid=<?php echo $cid; ?>">
